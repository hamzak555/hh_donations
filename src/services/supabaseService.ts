...
export class BinsService {
  ...
  // Driver-specific optimized query - only fetches bins assigned to specific driver
  static async getDriverBins(driverId: string): Promise<BinLocation[]> {
    const { data, error } = await supabase
      .from(TABLES.BINS)
      .select('id, binNumber, locationName, address, lat, lng, status, assignedDriver, fillLevel, sensorEnabled, created_at')
      .eq('assignedDriver', driverId)
      .order('created_at', { ascending: true })
...
export class DriversService {
  static async getAllDrivers() {
    const { data, error } = await supabase
      .from(TABLES.DRIVERS)
      .select('id, name, phone, email, licenseNumber, hireDate, status, assignedBins, vehicleType, totalPickups, notes, hasCredentials, password_hash, last_login, password_changed_at, assignedPickupRoutes, created_at, updated_at')
      .order('created_at', { ascending: true })
...
export class BalesService {
  static async getAllBales() {
    const { data, error } = await supabase
      .from(TABLES.BALES)
      .select('id, baleNumber, weight, contents, createdDate, location, status, pricePerKg, buyerInfo, soldDate, salePrice, paymentMethod, notes, notesTimeline, photos, containerNumber, created_at, updated_at')
      .order('created_at', { ascending: true })
...
export class PickupRequestsService {
  ...
  // Paginated query for large datasets - reduces admin dashboard load
  static async getPickupRequestsPaginated(page: number = 0, limit: number = 50) {
    const { data, error, count } = await supabase
      .from(TABLES.PICKUP_REQUESTS)
      .select('id, name, phone, address, date, status, assignedDriver, created_at', { count: 'exact' })
      .range(page * limit, (page + 1) * limit - 1)
      .order('created_at', { ascending: false })

    if (error) {
      console.error('Error fetching paginated pickup requests:', error)
      throw error
    }
  ...
  // Driver-specific optimized query - only fetches requests assigned to specific driver
  static async getDriverPickupRequests(driverId: string) {
    const { data, error } = await supabase
      .from(TABLES.PICKUP_REQUESTS)
      .select('id, name, phone, address, date, time, location, status, assignedDriver, created_at')
      .eq('assignedDriver', driverId)
